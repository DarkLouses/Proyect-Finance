                            ...
                            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                                ...
                                <li aria-current="page">
                                    <div class="flex items-center">
                                        ...
                                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Ingresos</span>
                                    </div>
                                </li>
                            </ol>
                        </nav>
                        <div class="w-full h-full flex justify-end mr-3 items-end">
                            <a href="{{ route('incomes.create') }}" class="h-full bg-green-500 hover:bg-green-600 text-gray-800 font-bold py-2 px-4 rounded" autofocus>Nuevo Ingreso</a>
                        </div>
                    </div>
                    <div class="w-full">
                                ...
                                <tbody class="bg-white">
                                    @forelse($incomes as $income)
                                        <tr class="even:bg-gray-200 odd:bg-gray-10 ">
                                            <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-900">{{ $income->bank->name }}</td>
                                            <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-900">{{ $income->description }}</td>
                                            <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-900">{{ $income->amount }} $</td>
                                            <td class="p-4 whitespace-nowrap text-sm font-normal text-gray-900">{{ \Carbon\Carbon::parse($income->date)->format('d-m-Y - H:i:s') }}</td>
                                            <td class="flex justify-center p-4 whitespace-nowrap text-sm font-normal text-gray-900">
                                                <a href="{{ route('incomes.edit', $income->id) }}" class="table-cell bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
