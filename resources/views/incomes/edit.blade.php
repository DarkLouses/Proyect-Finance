                    ...
                    <div class="flex mb-5">
                        <nav class="flex w-fit px-5 py-3 text-gray-900 border border-gray-200 rounded-lg bg-gray-50" aria-label="Breadcrumb">
                            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                                ...
                                <li class="inline-flex items-center">
                                    <a href="{{ route('incomes.index') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 ">Ingresos</a>
                                </li>
                                    ...
                                    <div class="w-fit">
                                            ...
                                            <label for="description">Banco
                                                <select name="bank_id" autofocus class="bg-white border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                                    @foreach($banks as $bank)
                                                        <option value="{{ $bank->id }}" {{ $bank->id == $income->bank_id ? 'selected' : '' }}>{{ $bank->name }}</option>
                                                    @endforeach
                                                </select>
                                            </label>
                                        </div>
                                        <div class="m-auto mt-6">
                                            <label for="description" class="m-auto mt-3">Descripcion
                                                <input type="text" name="description" class="mt-1 p-2 w-full border rounded-md" value="{{ $income->description }}">
                                            </label>
                                        </div>
                                        <div class="grid grid-cols-2 mt-6">
                                            <label for="amount" class="mr-2">Monto
                                                <input type="text" name="amount" class="mt-1 p-2 w-full border rounded-md" value="{{ $income->amount }}">
                                            </label>
                                            <label for="date">Fecha
                                                <input type="datetime-local" name="date" class="mt-1 p-2 w-full border rounded-md" value="{{ $income->date }}">
                                            </label>
                                        </div>
                                        <div class="mt-6">
